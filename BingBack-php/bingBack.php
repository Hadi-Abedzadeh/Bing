...
while(true){
    ...
    foreach ($links as $tag) @$ret[urldecode($tag->getAttribute('src'))] = $tag->childNodes->item(0)->nodeValue;	

	$ret = (array_keys($ret)); flush();
	if(strlen($ret[1]) <= 1 ) break;
	file_put_contents("$dir/".date('Ymd', $date).".jpg", file_get_contents(str_replace('//', 'https://', $ret[1])), LOCK_EX);
	print("$dir/".date('Ymd', $date).".jpg\t" .date('Y/m/d H:i:s ').microtime().PHP_EOL); $increment++;
	
	if(date('Ymd', $date) >= date('Ymd')) break;
}
echo "\tOperation is successfully\n\n\n";
